...
@section('content')
<?php $title = "فيديوهات"; ?>
@if(count($greetingImgs)> 0 && isset($greetingAudiosForOcc) && count($greetingAudiosForOcc)>0 )
...
<div class="gallery check">
    ...
    <a href="{{url('InputVideo/'.$greetingImg->id.'/'.UID())}}" class="occasion_{{$greetingImg->occasion_id}} @if(Occasion()!= $greetingImg->occasion_id ) hide @endif"><img src="{{asset("$greetingImg->path")}}"></a>
