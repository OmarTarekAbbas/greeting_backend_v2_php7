...
<div class="chooseitem filter" id="chooseimageitem">
    <div class="owl-carousel owl-theme">
        @foreach($snap_Occasions as $occasion)
        <div class="item @if(Occasion()==$occasion->id) active1 @endif" action='inactive'>
          @if(isset($_REQUEST['SearchKey']))
            <a href="{{url('/list_snap_v1/'.$occasion->id.'/'.UID())}}" onclick="window.location.replace('{{url('/list_snap_v1/'.$occasion->id.'/'.UID().'?SearchKey='.$_REQUEST['SearchKey'])}}')">
          @else
            <a href="{{url('/list_snap_v1/'.$occasion->id.'/'.UID())}}" onclick="window.location.replace('{{url('/list_snap_v1/'.$occasion->id.'/'.UID())}}')">
          @endif
...
<ul class="sounds snapchat check list-unstyled">
        ...
        @foreach($Snapdata[$oc_id] as $key => $img)
        <!-- Audio -->
        <li data-type="<?= $img->rbt_id ? 1 : 0 ?>" class="snap occasion_{{$img->occasion_id}} @if(Occasion()!= $img->occasion_id ) sound_hide @endif" >
            ...
            @if($img->rbt_id)
            <a class="icon"  href="sms:{{$rbt_sms}}<?php echo $Att; ?>{{$codes[0][$key]}}"><i class="fas fa-cart-plus"></i></a>
            ...
            @endif
             ...
             <div class="view">
                    <i id="eye" class="fas fa-eye"></i> <span> {{$img->popular_count}}</span>
                  </div>
    ...
    $(window).on("scroll", function () {
      ...
      if(typeof action == "undefined")
        var action = $('#chooseimageitem .active1 .item').attr('action');
            ...
            var occasion_id = '{{$Snapdata[$oc_id][0]->occasion_id}}';
